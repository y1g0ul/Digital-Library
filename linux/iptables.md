...
> **Примечание:** В современных дистрибутивах (например, Ubuntu) `iptables` часто работает поверх **nftables** через режим совместимости (`iptables-nft`). Для новых конфигураций рекомендуется использовать `nft`, но `iptables` по-прежнему широко встречается и важно знать его синтаксис.